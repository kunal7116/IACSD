 ...
 */
	...
	public void init() throws ServletException {
		...
		try {
			...
			//dependent:servlet, dependency:user dao
			//dependent Object is creating it's own dependency
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) {
				...
				// valid login -- 1. Create a cookie
				//java.servlet.http.cookie(String cookieName,string cookieVal)
				Cookie c1 = new Cookie("user_details",user.toString());
				//2. send the cookie to client, using respone header
				...
				if (user.getRole().equals("voter")) {
					 ...
					 * 2.sends temp redirect resp - SC 302,Header - 1
					 *Location-"candidate_list",
					 *2. Set-Cookie - cookie name -value
					 *resp body -empty
					 *3.Client browser -chks privacy settings
					 *cookies blocked -- cookies won't 
					 *be stored --cant't remember the client
					 *aacepted --cookie age --def value -1=>
					 *saves it in the cache(cookie storage)
					 * 4.Client browser - sends NEXT req (redirect)
					 * URL- http://host:portal/day2.1/candidate_list
